...
\Def \key{Zahlmengen}: $\NN = \{1, 2, \ldots\}$, $\NN_0 = \NN \cup \{0\}$, $\ZZ$, $\QQ$, $\RR$.
...
Mit $a, b \in \RR$ ist das Intervall \key{beschränkt}; mit $\pm\infty$ als Grenze
\key{unbeschränkt}, z.B. $\RR = (-\infty, \infty)$ und $(-\infty, b) = \{x \in \RR \, | \, x < b\}$
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item[(M3)] $x \cdot x^{-1} = 1$ für $x \ne 0$ (inverses Element)
...
$z = a + ib$ heisst Normal-/arithmetische/kartesische Form. $a = \Re(z)$ \key{Realteil},
...
\begin{itemize}
    ...
    \item $z + \overline{z} = 2\Re(z)$ und $z - \overline{z} = 2i\Im(z)$
...
\Satz Aus der Exponentialreihe $e^x = \sum_{k=0}^{\infty}\frac{1}{k!}x^k$ folgt mit $x = it$
die \key{Eulersche Formel}:
$$e^{it} = \cos(t) + i\sin(t)$$
...
\Satz \key{Wurzeln in $\CC$}: Lösungen von $z^n = w = re^{i\varphi}$:
$z_k = r^{\frac{1}{n}}e^{i\frac{\varphi + 2k\pi}{n}}$ wobei $0\le k < n$

\Satz \key{Fundamentalsatz der Algebra}: $p(z) = \sum_{k=0}^{n} a_k z^k$, $a_k \in \CC$, $a_n \ne 0$, $n\ge 1$
kann als $p(z) = a_n\prod_{k=1}^{n}(z-z_k)$ geschrieben werden, wobei $z_k$ die Nullstellen
von $p(z)$ (mit Vielfachheit) sind

\Satz $p(z)$ mit $a_k \in \RR$ $\implies$ Nullstellen treten in komplex konjugierten Paaren auf