...
\Def \key{Funktion}: Input $\to$ 1 Output aus \key{Zielbereich} ($\operatorname{codomain}(f)$)
\begin{align*}
    f: \operatorname{domain}(f) & \to \operatorname{codomain}(f) \\
    x & \mapsto y = f(x)
\end{align*}

\Def \key{Definitionsbereich} $\DD(f) = \operatorname{domain}(f)$: alle mögliche Inputs

\Def \key{Wertebereich}/\key{Bildmenge} $\WW(f) = \operatorname{image}(f)$: realisierte Outputs
...
\Def \key{Identität} (\key{Identitätsfunktion}): $\forall x \in X : \operatorname{id}_X(x) = x$

\Satz Falls $f: X \to Y$ bijektiv, ex. eindeutiges $g: Y \to X$ mit $g \circ f = \operatorname{id}_X$ und $f \circ g = \operatorname{id}_Y$.
...
$$\forall \varepsilon > 0 \, \exists \delta > 0 : x \in \DD(f) \cap (x_0 - \delta, x_0 + \delta) \backslash \{x_0\} \implies |f(x) - L| < \varepsilon$$
$L = \lim\limits_{\substack{x\ge x_0 \\ x \to x_0}} f(x)$ ist \key{der rechtsseitige Grenzwert} falls
$$\forall \varepsilon > 0 \, \exists \delta > 0 : x \in \DD(f) \cap [x_0, x_0 + \delta) \implies |f(x) - L| < \varepsilon$$
$L = \lim\limits_{\substack{x\le x_0 \\ x \to x_0}} f(x)$ ist \key{der linksseitige Grenzwert} falls
...
$$\forall N > 0 \, \exists \delta > 0 : x \in \DD(f) \cap (x_0 - \delta, x_0 + \delta) \backslash \{x_0\} \implies f(x) \ge N$$
$f$ hat in $x_0$ den \key{uneigentlichen Grenzwert} $\lim\limits_{x \to x_0}f(x) = -\infty$ falls
$$\forall N > 0 \, \exists \delta > 0 : x \in \DD(f) \cap (x_0 - \delta, x_0 + \delta) \backslash \{x_0\} \implies f(x) \le -N$$
...
\Def Exponentialfunktion mit \key{Anfangswert} $z_0$ und \key{Wachstumfaktor}/\key{Basis} $a > 0$: $z = f(t) = z_0 \cdot a^t = z_0 \cdot e^{kt}$ mit $k = \ln(a)$.
...
\begin{itemize}
    ...
    \item $\tan$ hat für $x = \pm \frac{(2n+1)\pi}{2}$, $n \in \NN_0$ eine \key{vertikale Asymptote}
...
$a_s$ heissen \key{Koeffizienten}; falls $a_n \ne 0$, ist $n$ der \key{Grad} des Polynoms.

\Def $y = f(x) = \frac{p(x)}{q(x)}$, $p(x)$ und $q(x) \ne 0$ Polynome, heisst \key{rationale F.}
...
\begin{itemize}
    ...
    \item Falls $f: I \subset \RR \to \operatorname{image}(f)$, $g: \operatorname{image}(f) \to \RR$ ist
...
\Satz \key{Zwischenwertsatz}: $f: [a, b] \to \RR$ stetig, $c$ zwischen $f(a)$ und $f(b)$.
...
\vspace{-0.6em}
\begin{itemize}
    \item \key{lokales Maximum} falls $\exists \delta > 0 : f(x_0) \ge f(x)$ für alle $x \in D \cap (x_0 - \delta, x_0 + \delta)$
    \item \key{lokales Minimum} falls $\exists \delta > 0 : f(x_0) \le f(x)$ für alle $x \in D \cap (x_0 - \delta, x_0 + \delta)$
