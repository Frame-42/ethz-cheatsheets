...
isolierten Punkte (d.h. jedes $x \in D$ ist Häufungspunkt von $D \backslash \{x\}$).
...
Falls für alle $x \in \DD(f)$ differenzierbar, $f$ \key{differenzierbar}
...
$\lim\limits_{\substack{h > 0 \\ h \to 0}}\frac{f(a+h) - f(a)}{h}$, analog links
$\lim\limits_{\substack{h < 0 \\ h \to 0}}\frac{f(a+h) - f(a)}{h}$
...
$C^\infty(D) := \cap_{n=0}^\infty C^n(D)$ sind die \key{glatten Funktionen}
...
\begin{itemize}
    ...
    \item $(f\cdot g)^{(n)}(x_0) = \sum_{k=0}^{n} \binom{n}{k} \cdot f^{(k)}(x_0) \cdot g^{(n-k)}(x_0)$
\end{itemize}
...
\Satz \key{Ableitung der Umkehrfunktion}: $f: D \to E \subset \RR$ stetig, bijektiv,  und an Stelle $x_0$ diff.bar mit $f'(x_0) \ne 0$,
...
existiert \key{Zwischenwert} $\xi$ zwischen $x_0$ und $x$ so dass für $R_n := f(x) - T_n(x, x_0)$ gilt:
...
\begin{itemize}
    ...
    \item $\ln (1 + x) = \sum_{k=1}^{\infty} (-1)^{k-1} \frac{x^k}{k} = x - \frac{x^2}{2} + \frac{x^3}{3} - \cdots$ für $-1 < x \le 1$
    ...
    \item $(1 + x)^\alpha = \sum_{k=0}^{\infty} \binom{\alpha}{k} x^k$, $\alpha \in \RR$, wobei $\binom{\alpha}{k} = \frac{\alpha(\alpha-1)\cdots(\alpha-k+1)}{k!}$ für $|x| < 1$
\end{itemize}
