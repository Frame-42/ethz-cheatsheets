...
Nun für die allgemeine Lösung der DG: sei $p(\lambda)$ vom Grad $n$ mit $r$ reellen
Nullstellen $\lambda_1, \ldots, \lambda_r$ (Multiplizität $m_i$) und $s$ \textit{Paaren}
komplexer Nullstellen $a_j \pm ib_j$ (Multiplizität $m_j$). Für beliebige Konstanten
$C_{ip}, A_{jq}, B_{jq} \in \RR$ gilt:
\vspace{-0.6em}
\begin{itemize}
    \item Falls alle Nullstellen reell ($s = 0$):
    $$u(x) = \sum_{i=1}^{r} \sum_{p=0}^{m_i-1} C_{ip} \, x^p e^{\lambda_i x}$$
    \item Allgemein, mit reellen und komplexen Nullstellen:
    \vspace{-1.2em}
    \begin{align*}
        u(x) = &\sum_{i=1}^{r} \sum_{p=0}^{m_i-1} C_{ip} \, x^p e^{\lambda_i x} \\
        &+ \sum_{j=1}^{s} \sum_{q=0}^{m_j-1} x^q e^{a_j x}\big(A_{jq} \sin(b_j x) + B_{jq} \cos(b_j x)\big)
    \end{align*}
\end{itemize}
