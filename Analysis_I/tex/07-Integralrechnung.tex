...
$F'(x) = f(x) \, \forall x \in I$ nennt man \key{Stammfunktion} von $f$ auf $I$. Das 
Bestimmen von $F(x)$ aus $f(x)$ nennt man (\key{unbestimmte}) \key{Integration}

\Def Die Familie von Funktionen $F(x) + C$ (wobei $F$ Stammfunktion vom \key{Integrand} $f$ ist)
...
\begin{enumerate}
    ...
    \item Löse für $A, B, C, \ldots$ (Nullstellen einsetzen oder Gleichungssystem)
...
\begin{tabular}{l l l}
    ...
    $\tan^2 x$         &                     & $\sec^2 x - 1 = \tan^2 x$ \\
    $\cot^2 x$         &                     & $\csc^2 x - 1 = \cot^2 x$ \\
...
In diesem Fall wird der Wert das \key{Riemann-Integral von $f$} genannt:
$$\int_a^b \hspace{-4pt}f(x) dx := \sup \mathcal L (f) = \inf \mathcal U (f)$$
$a$ und $b$ sind \key{untere}/\key{obere Integrationsgrenze} und $f$ der \key{Integrand}

\Satz \key{Eigenschaften des Riemann-Integrals}: $f,g: [a,b] \to \RR$ (Riemann-) integrierbar,
$\alpha, \beta \in \RR$:
\vspace{-0.6em}
\begin{itemize}
    ...
    \item $\displaystyle \left|\int_a^b \hspace{-4pt} f(x) dx\right| \le \int_a^b \hspace{-4pt} |f(x)| dx$ (Dreiecksungleichung)
...
gegen $f: [a, b] \to \RR$ konvergiert. Dann $f$ int.bar und
...
$$\int_a^b \hspace{-4pt} f(t) dt = F(t)\Big|_a^b = F(b) - F(a)$$
...
und $g: J \to \RR$ stetig. Für $[a, b] \subset I$ gilt $f'(x) > 0$ für alle $x \in [a, b]$
(also $f$ streng mon. wachsend) und $f^{-1}: [f(a), f(b)] \to \RR$ die Inverse von $f\Big|_{[a,b]}$. Dann gilt
