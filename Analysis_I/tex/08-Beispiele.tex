...
\begin{enumerate}
    \item Löse $y_h''' - 2y_h'' = 0$ (char. Polynom $\lambda^3 - 2\lambda^2 = \lambda^2(\lambda - 2)$)
        ...
        \end{align*}
    \item Finde $y_p$ und Ableitungen mit Ansätzen (beide sind Spezialfälle):
        \vspace{-1em}
        \begin{align*}
            y_p &= A x e^{2x} + B x^2 \\[-0.4em]
            y_p' &= A e^{2x} + 2 A x e^{2x} + 2 B x \\[-0.4em]
            y_p'' &= 4Ae^{2x} + 4A x e^{2x} + 2B \\[-0.4em]
            y_p''' &= 12 A e^{2x} + 8A x e^{2x}
        \end{align*}
    \item Substitution in der DG um $A, B$ zu bestimmen:
        \begin{align*}
            y_p''' - 2y_p'' &= 4e^{2x} + 12 \\[-0.3em]
            12A e^{2x} + 8Axe^{2x} - 8Ae^{2x} - 8Axe^{2x} - 4B &= 4e^{2x} + 12 \\[-0.6em]
            4A e^{2x} - 4B &= 4e^{2x} + 12 \\[-0.6em]
            \implies A = 1, B = -3 \hspace{60pt}& \\[-0.6em]
...
\begin{minipage}{\linewidth}
\Bsp \key{Partielle Integration + Substitution}: Lösung von $\displaystyle \int \arcsin(x) dx$
\vspace{-0.6em}
\begin{align*}
...
\begin{enumerate}
    ...
    \item Lösen durch Einsetzen von $x = 0, \pm 1$: $A = C = 1, B = -1$
